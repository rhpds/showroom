...
NOTE: This requires `zerotouch-automation` `>= v1.3.2` (the runtime automation image is now pinned to this version by default -- see `ocp4_workload_showroom_runtime_automation_image`), which reads the kubeconfig Secret name from the `RUNTIME_AUTOMATION_KUBECONFIG_SECRET_NAME` setting (default `runtime-automation-kubeconfig`, matching this role's default).
